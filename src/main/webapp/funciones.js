...
function llenarComboOPE()
{
	request = crearXMLHttpRequest( );
	request.open("GET","Combos?accion=comboOperadora",true);
	request.onreadystatechange=manejadorOPE;
	request.send(null);
}

function manejadorOPE()
{
    var resp=request.responseXML;
    var x=resp.getElementsByTagName("nombre");
    var combo=document.getElementById("operadoras");
    for(i=0;i<x.length;i++){
        var option=document.createElement("option");
        option.appendChild(document.createTextNode(x[i].firstChild.nodeValue));
        combo.appendChild(option);
...
function llenarComboRELA()
{
	request = crearXMLHttpRequest( );
	request.open("GET","Combos?accion=comboRelacion",true);
	request.onreadystatechange=manejadorRELA;
	request.send(null);
}

function manejadorRELA()
{
    var resp=request.responseXML;
    var x=resp.getElementsByTagName("nombre");
    var combo=document.getElementById("relaciones");
    for(i=0;i<x.length;i++){
        var option=document.createElement("option");
        option.appendChild(document.createTextNode(x[i].firstChild.nodeValue));
        combo.appendChild(option);
