   ...
   static final long serialVersionUID = 1L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			s.setAttribute("usuarioIniciado", ZarCuenta.obtenerNombrexCorreo(username));
			...
			request.setAttribute("error", "Usuario o Correo electronico incorrecto" );
			
		} catch (IncorrectCredentialsException ex) {
			//password provided did not match password found in database
			//for the username provided
			ex.printStackTrace();
			request.setAttribute("error", "Contrasenia incorrecta" );
		}
		
		catch (Exception ex) {
			
			ex.printStackTrace();			
			//request.setAttribute("error", ex.getMessage()+" Login NOT SUCCESSFUL - cause not known!");
